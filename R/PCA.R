...
olink_pca <- function(se, colby = NULL, shape = NULL, metavars = NULL,
  ...
  while (class(p <- try(pca(x,
    metadata = colData(se), scale = TRUE,
    removeVar = removeVar
  ...
  ## Loading plot
  ans[["loading plot"]] <- plotloadings(p,
    components = head(p$components, 5),
  ...
  ## eigencorplot
  if (!is.null(metavars)) {
    ans[["eigencorplot"]] <- tryCatch(eigencorplot(p,
      metavars = metavars,
      corFUN = "pearson",
